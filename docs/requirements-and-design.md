...
工具必须方便拷贝到不同机器使用：不依赖 Python、Flask、pip、虚拟环境、数据库或后端服务。浏览器负责完成 nmon 文件读取、解析、统计、图表渲染和报告导出。
...
如果浏览器限制本地文件能力，可以使用任意静态文件服务器托管当前目录，例如：

```bash
python3 -m http.server 8765
```

该命令只负责提供静态文件，不参与解析、汇总和绘图。所有数据处理都在浏览器内完成。
...
图表使用 ECharts 渲染。默认从 CDN 加载：

```html
https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js
```

离线使用时，可以将 `echarts.min.js` 放到 `assets/echarts.min.js`，页面会自动作为兜底加载。
...
- ECharts 默认来自 CDN，请求 CDN 只加载脚本，不上传 nmon 内容。
- 如需严格离线或内网环境使用，应将 `echarts.min.js` 放到 `assets/echarts.min.js`。
